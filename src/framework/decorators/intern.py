"""
Kafka-runtime policy decorators (framework-internal).

These decorators are METADATA-ONLY — they attach lightweight configuration to a
function via ``__policy_*__`` attributes.  The Kafka ETL runtime
(``framework.etl.framework_etl``) reads those attributes and applies the
policies at the transport level:

- ``retry_to_dlq``  — requeue up to N attempts inside Kafka, then route to a
  DLQ topic.  The retry counter is embedded in the message payload so retries
  survive process restarts.
- ``circuit_breaker`` — open after N consecutive failures, pausing the
  worker's ``TopicPartition``s while open, then auto-reset.
- ``rate_limit`` — token-bucket throttle on job dispatch rate into the
  worker's ``ThreadPoolExecutor``.

These decorators have **no effect** outside the Kafka runtime.  They do not
wrap the function and do not intercept calls.  This means decorated functions
can be called directly from tests or HTTP handlers without triggering
retry/rate-limit/circuit-breaker logic.

For function-level call wrapping (tenacity / pybreaker / limits) that works
in *any* context, see :mod:`framework.decorators.common`.
"""
